...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
   ...
   case 'show_value':
    ...
    // Em theste.php, na parte “default” do switch($action):

// defina quantos itens por página
$limit = 10;
// descubra a página atual (via GET ?page=)
$page  = isset($_GET['page']) && is_numeric($_GET['page']) ? (int) $_GET['page'] : 1;
$offset = ($page - 1) * $limit;

// conte o total de produtos
$totalItems = $produtoModel->countAll();
$totalPages = (int) ceil($totalItems / $limit);

// pegue apenas os produtos da página atual
$produtos = $produtoModel->getPaged($limit, $offset);
    ...
    if (!empty($_POST['codigo_barras'])) {
        ...
        if ($produtoExistente) {
            ...
            header("Location: theste.php?msg=" . urlencode($mensagem) . "&msg_type=danger&error_duplicate=1");
            exit;
        }
    }
    ...
    if ($produtoModel->create($dados)) {
        ...
        header("Location: theste.php?msg=Produto e variações criados com sucesso!&msg_type=success");
    } else {
        header("Location: theste.php?msg=Erro ao criar produto!&msg_type=danger");
    }
    exit;
        case 'update':
          if (empty($_POST['codigo_barras'])) {
    ...
    if (! $produtoModel->update($id_produto, $dados)) {
        header("Location: theste.php?msg=Erro ao atualizar produto!&msg_type=danger");
        exit;
    }
        ...
        // redirect correto
        if ($id_variacao > 0) {
            header("Location: theste.php?action=variacoes&id={$parentId}&msg=" . urlencode($msg) . "&msg_type={$msg_type}");
        } else {
            header("Location: theste.php?msg=" . urlencode($msg) . "&msg_type={$msg_type}");
        }
        exit;
...
if ($qtd > 0) {
    ...
    $despesaModel->createDespesa(
        ...
        $itens
                ...
                header("Location: theste.php?action=variacoes&id=$id_produto&msg=" . urlencode($msg) . "&msg_type=$msg_type");
                exit;
    }
}
...
switch ($action) {
    case 'delete':
        if ($id_produto > 0) {
            if ($produtoModel->delete($id_produto)) {
                header("Location: theste.php?msg=Produto excluído com sucesso!&msg_type=success");
            } else {
                header("Location: theste.php?msg=Erro ao excluir produto!&msg_type=danger");
            }
        } else {
            header("Location: theste.php?msg=ID de produto inválido!&msg_type=warning");
        }
        exit;
    ...
    case 'delete_variacao':
        if ($id_variacao > 0) {
            ...
            header("Location: theste.php?action=variacoes&id=$id_produto&msg=$msg&msg_type=$msg_type");
        } else {
            header("Location: theste.php?action=variacoes&id=$id_produto&msg=ID de variação inválido!&msg_type=warning");
        }
        exit;
}
...
<head>
    ...
    @media (max-width: 992px) {
      ...
      .drawer-overlay.show {
        z-theste: 1999;              /* opcional, se usar overlay */
      }
    ...
    body.dark-mode {
      --body-bg: #121212;
      --dark: #ffffff; /* Changed from #e0e0e0 to white for better contrast */
      --gray: #adb5bd;
      --gray-dark: #6c757d;
      --gray-light: #2d2d2d;
    ...
    @media (max-width: 768px) {
      ...
      :root {
        --topbar-height: 0; /* Hide desktop topbar */
      }
   ...
   <div class="app-container" id="appContainer">
            ...
            <div class="topbar">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div class="topbar-search">
                        <input type="text" class="form-control" id="searchInput" placeholder="Buscar produtos, variações...">
                        <i class="bi bi-search"></i>
                        <div id="suggestions" class="suggestions-dropdown"></div>
                    </div>
                
                <div class="d-flex flex-row justify-content-center align-items-center gap-3">
                  <div class="topbar-actions" style="z-theste: 9999;">
        ...
        <?php include_once '../../frontend/includes/darkmode.php'?>
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="theste.php"><i class="bi bi-house-door me-1"></i>Home</a></li>
                    <?php switch ($action):
                        case 'create': ?>
                            <li class="breadcrumb-item"><a href="theste.php">Produtos</a></li>
                        ...
                        case 'variacoes':
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Variações:
                                <?php echo $produto ? $produto['nome'] : 'Produto'; ?></li>
                            <?php break;
                        default: ?>
    ...
    case 'create': ?>
        ...
        <div class="card fade-in-up" data-aos="fade-up">
            ...
            <div class="card-body">
                <form action="theste.php?action=store" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="d-flex justify-content-end mt-4">
                        <a href="theste.php" class="btn btn-outline-secondary me-2 hover-lift">
    ...
    <?php break;
                    ...
                    case 'edit':
                        ...
                        if (!$produto) {
                            ...
                            echo '<a href="theste.php" class="btn btn-primary hover-lift"><i class="bi bi-arrow-left"></i> Voltar para a lista</a>';
                            break;
                        }
                        ?>
                        ...
                        <div class="card fade-in-up" data-aos="fade-up">
                            ...
                            <div class="card-body">
                                <form action="theste.php?action=update&id=<?php echo $id_produto; ?>" method="POST" enctype="multipart/form-data" class="needs-validation" novalidate>
                                    <div class="row mb-3">
                                        <div class="col-md-8">
                                            <label for="nome" class="form-label">Nome do Produto</label>
                                            <input type="text" class="form-control" name="nome" id="nome" value="<?php echo $produto['nome']; ?>" required>
                                            <div class="invalid-feedback">Por favor, informe o nome do produto.</div>
                                        </div>
                                        <div class="col-md-4">
                                            <label for="codigo_barras" class="form-label">Código de Barras</label>
                                            <input type="text" class="form-control" name="codigo_barras" id="codigo_barras" value="<?php echo $produto['codigo_barras']; ?>">
                                        </div>
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <?php if (!empty($produto['foto'])): ?>
                                            <div class="mt-2">
                                                <img src="<?php echo $produto['foto']; ?>" alt="Imagem do Produto" class="img-thumbnail hover-shadow" style="max-height: 100px;">
                                            </div>
                                        <?php endif; ?>
                                    </div>
                                    ...
                                    <div class="row mb-3">
                                        <div class="col-md-6">
                                            ...
                                            <div class="input-group">
                                                <span class="input-group-text">R$</span>
                                                <input type="number" step="0.01" class="form-control" name="preco_custo" id="preco_custo" value="<?php echo $produto['preco_custo']; ?>">
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <label for="preco_venda" class="form-label">Preço de Venda</label>
                                            <div class="input-group">
                                                <span class="input-group-text">R$</span>
                                                <input type="number" step="0.01" class="form-control" name="preco_venda" id="preco_venda" value="<?php echo $produto['preco_venda']; ?>">
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="row mb-3">
                                        <div class="col-md-4">
                                            <label for="estoque_min" class="form-label">Estoque Mínimo</label>
                                            <input type="number" class="form-control" name="estoque_min" id="estoque_min" value="<?php echo $produto['estoque_min']; ?>">
                                        </div>
                                        <div class="col-md-4">
                                            <label for="estoque_max" class="form-label">Estoque Máximo</label>
                                            <input type="number" class="form-control" name="estoque_max" id="estoque_max" value="<?php echo $produto['estoque_max']; ?>">
                                        </div>
                                        <div class="col-md-4">
                                            <label for="estoque_atual" class="form-label">Estoque Atual</label>
                                            <input type="number" class="form-control" name="estoque_atual" id="estoque_atual" value="<?php echo $produto['estoque_atual'] ?? 0; ?>">
                                        </div>
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="input-group">
                                            <span class="input-group-text"><i class="bi bi-geo-alt"></i></span>
                                            <input type="text" class="form-control" name="localizacao_estoque" id="localizacao_estoque" value="<?php echo $produto['localizacao_estoque']; ?>">
                                        </div>
                                    </div>
                ...
                case 'variacoes':
                    ...
                    <div class="page-header" data-aos="fade-down">
                        ...
                        <a href="theste.php" class="btn btn-outline-primary hover-lift">
                    ...
                    <div class="row">
                        <div class="col-lg-4 order-lg-2 order-1">
                            <div class="card fade-in-up" data-aos="fade-left" style="animation-delay: 0.2s;">
                                <div class="card-header">
                                    <i class="bi bi-<?php echo $editando_variacao ? 'pencil-square' : 'plus-circle'; ?>"></i>
                                    <?php echo $editando_variacao ? "Editar Variação" : "Nova Variação"; ?>
                                </div>
                                <div class="card-body">
                                    <form method="POST" action="theste.php?action=store_variacao&id=<?php echo $id_produto; ?>"
                                        class="needs-validation" novalidate>
                                        <?php if ($editando_variacao && $variacaoEdicao): ?>
                                            <input type="hidden" name="id_variacao"
                                                value="<?php echo $variacaoEdicao['id_variacao']; ?>">
                                        <?php endif; ?>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <div class="input-group">
                                                ...
                                                id="var_cor"
                                                value="<?php echo $editando_variacao ? $variacaoEdicao['cor'] : ''; ?>">
                                            </div>
                                        </div>
                                        <div class="mb-3">
                                            ...
                                            <div class="input-group">
                                                ...
                                                id="var_tamanho"
                                                value="<?php echo $editando_variacao ? $variacaoEdicao['tamanho'] : ''; ?>">
                                            </div>
                                        </div>
                                        <div class="mb-3">
                                            ...
                                            <div class="input-group">
                                                ...
                                                id="var_sku"
                                                value="<?php echo $editando_variacao ? $variacaoEdicao['sku'] : ''; ?>">
                                            </div>
                                        </div>
                                        <div class="mb-3">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input type="number" step="0.01" name="preco_venda" class="form-control"
                                                    value="<?php echo $editando_variacao ? $variacaoEdicao['preco_venda'] : ''; ?>">
                                            </div>
                                        </div>
                                        <div class="mb-3">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input type="number" name="estoque_atual" class="form-control"
                                                    value="<?php echo $editando_variacao ? $variacaoEdicao['estoque_atual'] : ''; ?>">
                                            </div>
                                        </div>

                                        <div class="d-grid gap-2">
                                            <button type="submit"
                                                class="btn btn-<?php echo $editando_variacao ? 'primary' : 'success'; ?> hover-lift">
                                                <i
                                                    class="bi bi-<?php echo $editando_variacao ? 'check-circle' : 'plus-circle'; ?>"></i>
                                                <?php echo $editando_variacao ? "Atualizar" : "Adicionar Variação"; ?>
                                            </button>
                                            <?php if ($editando_variacao): ?>
                                                <a href="theste.php?action=variacoes&id=<?php echo $id_produto; ?>"
                                            ...
                                            <tbody>
                                                <?php if (!empty($variacoes)): ?>
                                                    <?php foreach ($variacoes as $var): ?>
                                                        <tr class="fade-in-up" data-aos="fade-up"
                                                            data-aos-delay="<?php echo 50 * $var['id_variacao']; ?>">
                                                            <td><?php echo $var['id_variacao']; ?></td>
                                                            <td>
                                                                <?php if (!empty($var['cor'])): ?>
                                                                    <span class="badge bg-light text-dark hover-scale">
                                                                        <i class="bi bi-palette me-1"></i> <?php echo $var['cor']; ?>
                                                                    </span>
                                                                <?php else: ?>
                                                            ...
                                                            <td>
                                                                <?php if (!empty($var['tamanho'])): ?>
                                                                    <span class="badge bg-light text-dark hover-scale">
                                                                        <i class="bi bi-rulers me-1"></i> <?php echo $var['tamanho']; ?>
                                                                    </span>
                                                                <?php else: ?>
                                                                    <span class="text-muted">-</span>
                                                                <?php endif; ?>
                                                            </td>
                                                            <td><?php echo $var['sku'] ?: '-'; ?></td>
                                                            <td class="fw-bold text-primary">R$
                                                                <?php echo number_format($var['preco_venda'], 2, ',', '.'); ?></td>
                                                            <td>
                                                                <?php
                                                                $estoque_class = 'success';
                                                                if ($var['estoque_atual'] <= $produto['estoque_min']) {
                                                                    $estoque_class = 'danger';
                                                                } elseif ($var['estoque_atual'] <= $produto['estoque_min'] * 2) {
                                                                    $estoque_class = 'warning';
                                                                }
                                                                ?>
                                                                <span class="badge bg-<?php echo $estoque_class; ?> hover-scale">
                                                                    <?php echo $var['estoque_atual']; ?> un
                                                                </span>
                                                            </td>
                                                        <td class="text-center">
                                                          <!-- Editar variação -->
                                                          <a href="#"
                                                             class="btn btn-sm btn-warning hover-lift me-1"
                                                             onclick="loadVariationEdit(<?= $var['id_variacao']; ?>)">
                                                            <i class="bi bi-pencil"></i>
                                                          </a>
                                                        
                                                          <!-- Excluir variação -->
                                                          <a href="theste.php?action=delete_variacao&id=<?= $id_produto; ?>&id_variacao=<?= $var['id_variacao']; ?>"
                                                             class="btn btn-sm btn-danger hover-lift me-1"
                                                             onclick="return confirm('Deseja excluir esta variação?')">
                                                            <i class="bi bi-trash"></i>
                                                          </a>
                                                        
                                                          <!-- Novo: Adicionar Estoque à variação -->
                                                          <button
                                                            type="button"
                                                ...
                                                <?php else: ?>
                                                    <tr>
                                                        <td colspan="7" class="text-center py-4">
                                                            <i class="bi bi-exclamation-circle text-muted d-block mb-2"
                                                                style="font-size: 2rem;"></i>
                                                            <p class="text-muted mb-2">Nenhuma variação cadastrada para este
                                                                produto.</p>
                                                            <p class="text-muted">Utilize o formulário ao lado para adicionar
                                                                variações.</p>
                                                        </td>
                                                    </tr>
                                                <?php endif; ?>
        ...
        $totalItems = $produtoModel->countAll();
        $totalPages = (int) ceil($totalItems / $limit);
        // busca só os produtos da página atual
        $produtos   = $produtoModel->getPaged($limit, $offset);
        // ==================

                    // Lista de produtos (página principal)
                    ...
                    <div class="page-header" data-aos="fade-down">
                        ...
                        <a href="theste.php?action=create" class="btn btn-primary hover-lift">
                    ...
                    <div class="d-md-none">
                        ...
                        <div class="mobile-actions">
                            ...
                            <div class="row g-2">
                                <div class="col-6">
                                    <a href="theste.php?action=create" class="btn btn-primary w-100 py-3">
                                        <i class="bi bi-plus-lg d-block mb-1" style="font-size: 1.5rem;"></i>
                                        Novo Produto
                                    </a>
                                </div>
                                <div class="col-6">
                                    <a href="../../theste.php" class="btn btn-outline-primary w-100 py-3">
                                    ...
                                    <tbody>
                                        <?php if (!empty($produtos)): ?>
                                            <?php foreach ($produtos as $prod):
                                                ...
                                                ?>
                                                <tr class="fade-in-up" data-aos="fade-up"
                                                    data-aos-delay="<?php echo 50 * $prod['id_produto']; ?>">
                                                    ...
                                                    <td class="fw-bold text-primary">R$
                                                        <?php echo number_format($prod['preco_exibicao'], 2, ',', '.'); ?></td>
                                                   ...
                                                   <td class="text-center">
                                                      <div class="d-inline-block text-center mx-1">
                                                        ...
                                                        <a href="theste.php?action=edit&id=<?php echo $prod['id_produto']; ?>"
                                                      ...
                                                      <div class="d-inline-block text-center mx-1">
                                                        ...
                                                        <a href="theste.php?action=variacoes&id=<?php echo $prod['id_produto']; ?>"
                                                      ...
                                                      <div class="d-inline-block text-center mx-1">
                                                        ...
                                                        <a href="#"
                                                           class="btn btn-sm btn-danger hover-lift"
                                        ...
                                        <?php else: ?>
                                            <tr>
                                                <td colspan="6" class="text-center py-5">
                                                    ...
                                                    <a href="theste.php?action=create" class="btn btn-primary pulse hover-lift">
...
<?php endif; ?>
                    ...
                    <div class="d-md-none mobile-product-list">
                                ...
                                <div class="mobile-product-item">
                                    ...
                                    <div class="mobile-product-details">
                                        ...
                                        <p class="mobile-product-price">R$
                                            <?php echo number_format($prod['preco_venda'], 2, ',', '.'); ?></p>
                        ...
                        <?php else: ?>
                            <div
                                ...
                                <a href="theste.php?action=create" class="btn btn-primary">
                                    <i class="bi bi-plus-lg"></i> Cadastrar Produto
                                </a>
                            </div>
                        <?php endif; ?>
    ...
    <div class="bottom-nav d-block d-md-none d-flex justify-content-center align-items-center">
      <a href="theste.php" class="bottom-nav-item <?php echo in_array($action, ['list','create','edit','variacoes'])?'active':''; ?>">
      ...
      <a href="../vendas/theste.php" class="bottom-nav-item <?php echo ($action=='vendas')?'active':''; ?>">
        <i class="fas fa-shopping-cart"></i>
        <span>Vendas</span>
      </a>
      <a href="../financeiro/theste.php" class="bottom-nav-item <?php echo ($action=='financeiro')?'active':''; ?>">
...
#btnVerValor:focus {
  ...
  color: #fff;
    ...
    <a href="theste.php?action=create" class="fab d-flex justify-content-center align-items-center">
        <i class="bi bi-plus-lg"></i>
    </a>
    
    <!-- Modal Adicionar Estoque -->
<div class="modal fade" id="addStockModal" tabtheste="-1" aria-labelledby="addStockModalLabel" aria-hidden="true">
...
<script>
  ...
  addStockModal.addEventListener('show.bs.modal', function (event) {
    ...
    form.action = 'theste.php?action=add_stock&id=' + produtoId;
...
<div class="modal fade" id="addStockVarModal" tabtheste="-1" aria-labelledby="addStockVarModalLabel" aria-hidden="true">
  <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="mb-3">
            ...
            <input type="text" class="form-control" id="varStockCost" name="preco_custo" placeholder="0,00" required>
          </div>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Fechar</button>
...
<div class="modal fade" id="passwordModal" tabtheste="-1">
...
document.getElementById('btnConfirmPassword').addEventListener('click', async () => {
  ...
  try {
    const res = await fetch('theste.php?action=show_value', {
      method: 'POST',
...
<script>
  ...
  addStockVarModal.addEventListener('show.bs.modal', event => {
    ...
    form.action       = 'theste.php?action=add_stock&id_variacao=' + varId;
    // limpa campos
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Adicionar efeitos aos elementos
        ...
        function fetchSuggestions(term) {
            fetch(`theste.php?ajax_search=1&term=${encodeURIComponent(term)}`)
                .then(res => res.json())
                .then(data => {
                    if (data.length > 0) {
                        let html = '';
                        data.forEach(item => {
                            html += `
                        <div class="suggestions-item" onclick="selectSuggestion(${item.id_produto}, '${item.nome.replace(/'/g, "\\'")}')">
                            <div class="d-flex align-items-center">
                                <div class="me-2 ${item.foto ? '' : 'bg-light'}" style="width: 30px; height: 30px; border-radius: 4px; overflow: hidden;">
                                    ${item.foto ? `<img src="${item.foto}" alt="${item.nome}" style="width: 100%; height: 100%; object-fit: cover;">` : '<i class="bi bi-image text-secondary d-flex align-items-center justify-content-center h-100"></i>'}
                                </div>
                                <div>
                                    <strong>${item.nome}</strong><br>
                                    <small class="text-muted">R$ ${parseFloat(item.preco_venda).toFixed(2).replace('.', ',')} | Cód: ${item.codigo_barras || 'N/A'}</small>
                                </div>
                            </div>
                        </div>
                    `;
                        ...
                        <div class="suggestions-item text-center py-3">
                            <i class="bi bi-search text-muted d-block mb-2" style="font-size: 1.5rem;"></i>
        ...
        // Quando clica numa sugestão, preenche o input com o nome
        function selectSuggestion(id, nomeProduto) {
            searchInput.value = nomeProduto;
            suggestionsBox.style.display = 'none';

            // Redireciona para a página de edição utilizando o id correto
            window.location.href = `theste.php?action=edit&id=${id}`;
        }

        // Função auxiliar para obter o ID do produto pelo nome
        function getProductIdByName(nome) {
            // Esta função seria implementada com base nos dados disponíveis
        ...
        function confirmDelete(id, nome) {
            if (confirm(`Tem certeza que deseja excluir o produto "${nome}"?`)) {
                window.location.href = `theste.php?action=delete&id=${id}`;
            }
        }
        ...
        function loadVariationEdit(idVariacao) {
            ...
            const form = document.querySelector('form[action*="store_variacao"]');
            if (form) {
                form.classList.add('pulse');
                setTimeout(() => form.classList.remove('pulse'), 1000);
            }

            // Fazemos AJAX para obter dados da variação
            fetch(`theste.php?ajax_variacao=1&id_variacao=${idVariacao}`)
                .then(response => response.json())
                .then(data => {
                    if (!data || !data.id_variacao) {
                        alert("Variação não encontrada!");
                        return;
                    }

                    // Preenche os campos do formulário de variação
                    if (!form) return;

                    // Define o hidden input "id_variacao" (se não existir, crie)
                    let hiddenIdVar = form.querySelector('input[name="id_variacao"]');
                    if (!hiddenIdVar) {
                        hiddenIdVar = document.createElement('input');
                        hiddenIdVar.type = 'hidden';
                        hiddenIdVar.name = 'id_variacao';
                        form.appendChild(hiddenIdVar);
                    }
                    hiddenIdVar.value = data.id_variacao;

                    // Define os demais campos com animação
                    const campos = [
                        { el: form.querySelector('input[name="cor"]'), val: data.cor || '' },
                        { el: form.querySelector('input[name="tamanho"]'), val: data.tamanho || '' },
                        { el: form.querySelector('input[name="sku"]'), val: data.sku || '' },
                        { el: form.querySelector('input[name="preco_venda"]'), val: data.preco_venda || 0 },
                        { el: form.querySelector('input[name="estoque_atual"]'), val: data.estoque_atual || 0 },
                        { el: form.querySelector('input[name="codigo_barras"]'), val: data.codigo_barras || '' }
                    ];

                    campos.forEach((campo, theste) => {
                        setTimeout(() => {
                            campo.el.value = campo.val;
                            campo.el.classList.add('pulse');
                            setTimeout(() => campo.el.classList.remove('pulse'), 500);
                        }, theste * 100);
                    });

                    // Muda o texto do botão de envio para "Atualizar"
                    ...
                    // Mudar o título do card
                    const cardHeader = form.closest('.card').querySelector('.card-header');
                    if (cardHeader) {
                        cardHeader.innerHTML = '<i class="bi bi-pencil-square"></i> Editar Variação';
                        cardHeader.classList.add('pulse');
                        setTimeout(() => cardHeader.classList.remove('pulse'), 1000);
                    }
                })
                .catch(err => {
                    console.error(err);
                    alert("Ocorreu um erro ao buscar a variação.");
                });
        }
  ...
  // Cria e exibe o modal usando Bootstrap
...
document.getElementById('desktopSidebarToggle').addEventListener('click', function(e) {
    ...
    // Adiciona/remove o overlay quando a sidebar é mostrada/escondida
    const overlay = document.querySelector('.drawer-overlay');
...
// Função para alternar a sidebar (usada em outros lugares)
function toggleDrawer() {
    const sidebar = document.querySelector('.sidebar');
    const overlay = document.querySelector('.drawer-overlay');
    ...
    <!-- Modal de Erro -->
<div class="modal fade" id="errorModal" tabtheste="-1" aria-labelledby="errorModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header bg-danger text-white">
        <h5 class="modal-title" id="errorModalLabel"><i class="bi bi-exclamation-triangle me-2"></i>Ocorreu um Erro</h5>
